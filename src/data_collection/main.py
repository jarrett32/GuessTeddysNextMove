...
async def initialize_blink(username, password):
    ...
    if os.path.exists("src/data_collection/blink_auth.json"):
        auth = Auth(await json_load("src/data_collection/blink_auth.json"), no_prompt=True)
        blink.auth = auth
        await blink.start()
    else:
...
async def take_screenshots(blink, cameras, output_path):
    while True:
        ...
        for camera in cameras:
            current_output_path = output_path.format(timestamp=timestamp, camera_name=camera.name) 
